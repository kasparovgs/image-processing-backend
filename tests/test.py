...
def get_image_processor_payload():
    ...
    return {"filter": { "name": "Blur", "parameters": { "sigma": 10 } }, "image": image_base64}
...
def test_create_task(auth_token):
    ...
    if len(payload) == 0:
        raise NotImplementedError("Choose one of the variants for payload!")
...
def test_task_status_and_result(auth_token):
    ...
    while retry >= 0:
        ...
        assert data['status'] == 'in_process', f"undefined status: {data['status']}!"
        retry -= 1
        time.sleep(3)
